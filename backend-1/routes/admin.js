...
// @route   GET /api/admin/download-database
// @desc    Download database backup (Admin only)
// @access  Private (Admin only)
router.get('/download-database', adminAuth, (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=saira-acad-backup-${timestamp}.db`);
        ...
        console.log(`✅ Database downloaded by admin: ${req.admin.username || req.admin.id}`);
